class RIASECTestEngine {
    ...
    initializeScores() {
        // Ensure all 6 dimensions are initialized
        const allDimensions = ['R', 'I', 'A', 'S', 'E', 'C'];
    ...
    handleKeyboardNavigation(e) {
        if (e.key === 'Escape') {
            ...
            // Number keys 1-3 for option selection
    ...
    displayCurrentSection() {
        const section = TEST_CONFIG.riasecSections[this.currentSection];
        const questions = RIASEC_QUESTIONS[section.id];
    ...
    // Go to next question
    nextQuestion() {
        const currentSection = TEST_CONFIG.riasecSections[this.currentSection];
        const questions = RIASEC_QUESTIONS[currentSection.id];
    ...
    // Go to next section
    nextSection() {
        if (this.currentSection < TEST_CONFIG.riasecSections.length - 1) {
            this.currentSection++;
            this.currentQuestion = 0;
            this.displayCurrentSection();
        } else {
    ...
    calculateResults() {
        ...
        // Ensure all 6 dimensions are processed
        const allDimensions = ['R', 'I', 'A', 'S', 'E', 'C'];
        
        allDimensions.forEach(dimension => {
            const maxPossibleScore = 30; // 10 questions * 3 points
    ...
    displayResults() {
        ...
        document.getElementById('results-title').textContent = 'તમારા RIASEC ટોપ પરિણામ';
        document.getElementById('results-subtitle').textContent = 'તમારા જવાબોના આધારે, અહીં તમારો સૌથી વધુ પ્રબળ કારકિર્દી રસ છે:';
        
        // Generate the interest scores HTML
        const interestScoresHtml = this.displayInterestScores();
        const careerRecommendationsHtml = this.displayCareerRecommendations();
        
        // Debug logging
        console.log('Displaying RIASEC Results:', {
            interestScoresHtml: interestScoresHtml,
            careerRecommendationsHtml: careerRecommendationsHtml
        });
        
        resultsContainer.innerHTML = `
            <div class="riasec-result">
                <h2>RIASEC કારકિર્દી રસ પ્રોફાઇલ</h2>
                <p class="subtitle">Your RIASEC Career Interest Profile</p>
                
                <div class="interest-scores">
                    <h4>🥇 તમારો ટોપ કારકિર્દી રસ:</h4>
                    ${interestScoresHtml}
                </div>
            ...
            <div class="interest-score top-result">
                <div class="score-header">
                    ...
                    <div class="score-display">
                        <span class="score-value">${topScore}/30</span>
    ...
    updateProgress() {
        ...
        TEST_CONFIG.riasecSections.forEach((section, sectionIndex) => {
            const sectionQuestions = RIASEC_QUESTIONS[section.id];
        ...
        document.getElementById('section-info').textContent = 
            `${section.name} (${this.currentSection + 1}/${TEST_CONFIG.riasecSections.length})`;
    }
    ...
    // Download results
    downloadResults() {
        let resultsText = 'RIASEC કારકિર્દી રસ પરીક્ષણ પરિણામો\n';
        resultsText += '================================\n\n';
        
        resultsText += '🥇 તમારો ટોપ કારકિર્દી રસ:\n';
        resultsText += '--------------------------------\n';
        ...
        resultsText += `🥇 ${topDimensionInfo.name} (${topDimensionInfo.english}): ${topScore}/30 (${topPercentage}%)\n`;
        resultsText += `   ${topDimensionInfo.description}\n`;
        resultsText += `   કારકિર્દી વિકલ્પો: ${topDimensionInfo.careers.join(', ')}\n\n`;
        ...
        a.href = url;
        a.download = 'riasec-career-interest-test-results.txt';
        a.click();
    ...
    validateTestCompletion() {
        const totalQuestions = TEST_CONFIG.riasecSections.length * 10;
